...
import { Shield, MapPin, Star, Camera, User, UserPlus, UserCheck, StickyNote, Save, Trash2 } from "lucide-react";
...
export default function Profile() {
  ...
  if (!profile) return (
    <Layout>
      <p className="text-slate-400 text-center mt-20">Profile not found.</p>
    </Layout>
  );
      ...
      <div className="max-w-3xl mx-auto space-y-5">
          ...
          <div className="flex items-start gap-5">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {profile.is_standby && (
                  <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600">Standby</span>
                )}
                {profile.is_ghost_mode && (
                  <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-500">Ghost Mode</span>
                )}
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="mb-3">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {profile.style_tags.map(t => (
                      <span key={t} className="text-xs px-2.5 py-1 rounded-full border font-display border-orange-200 text-orange-600 bg-orange-50">
                        {t}
                      </span>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {profile.editing_ecosystem.map(e => (
                      <span key={e} className="text-xs px-2.5 py-1 rounded-full border font-display border-slate-200 text-slate-600 bg-slate-50">
                        {e}
                      </span>
                    ))}
                  </div>
            ...
            {avgStyle ? (
              <div className="space-y-2.5">
                ...
                <p className="text-xs text-slate-400 mt-2">
                  {ratings.total_ratings} anonymous review{ratings.total_ratings !== 1 ? "s" : ""}
                </p>
              </div>
            ) : (
          ...
          {profile.gear_vault?.length > 0 && (
            <div className="p-5 rounded-xl border border-slate-200 bg-white md:col-span-2">
              <h3 className="text-sm font-semibold text-slate-900 font-display mb-3 flex items-center gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                {profile.gear_vault.map((g, i) => (
                  <span
                    key={i}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-display"
                    style={{
        ...
        {!isOwn && (
          <div data-testid="lead-notes-section" className="p-5 rounded-xl border border-orange-100 bg-white">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[10px] px-2 py-0.5 rounded-full font-display ml-auto bg-orange-50 text-orange-500">
                Only visible to you
              </span>
            </div>
