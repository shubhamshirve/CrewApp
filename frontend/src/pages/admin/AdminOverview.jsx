...
function StatCard({ label, value, icon: Icon, color = "#3B82F6", testId }) {
  return (
    <div
      ...
      className="p-5 rounded-2xl border border-border bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex items-center gap-4"
    >
  ...
  return (
    <AdminLayout>
      <div className="max-w-5xl mx-auto space-y-6">
        <div>
          <h1 className="text-4xl font-bold text-foreground font-display">Overview</h1>
        ...
        {loading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="h-20 rounded-2xl border border-border bg-slate-50 animate-pulse" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
          {[
            ...
            <a
              key={item.href}
              href={item.href}
              className="flex items-center justify-between p-4 rounded-2xl border border-border bg-white hover:border-blue-500/30 transition-all group shadow-[0_8px_30px_rgb(0,0,0,0.04)]"
            >
              <span className="text-sm text-slate-600 group-hover:text-foreground transition-colors font-display">{item.label}</span>
              <div className="flex items-center gap-2">
                ...
                <TrendingUp size={14} className="text-muted-foreground group-hover:text-blue-400 transition-colors" />
              </div>
            </a>
          ))}
        </div>
