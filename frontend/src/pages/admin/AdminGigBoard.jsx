...
export default function AdminGigBoard() {
  ...
  const STATUS_COLORS = {
    open: "text-emerald-600 bg-emerald-50 border border-emerald-200",
    partially_filled: "text-amber-600 bg-amber-50 border border-amber-200",
    filled: "text-slate-500 bg-slate-100 border border-slate-200",
    expired: "text-red-500 bg-red-50 border border-red-200",
    ...
    <AdminLayout>
      <div className="max-w-4xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">All public gig listings on the platform</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-slate-500 text-sm">No public gigs yet</p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-semibold text-slate-900 font-display">{g.title}</p>
                    <p className="text-xs text-slate-500">by {g.lead_name} · {g.event_type}</p>
                  ...
                  {g.status !== "cancelled" && g.status !== "filled" && (
                    <button
                      ...
                      className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs text-red-500 border border-red-200 hover:bg-red-50 transition-colors ml-4 flex-shrink-0"
                    >
                      <X size={11} /> Cancel
                    </button>
                  )}
