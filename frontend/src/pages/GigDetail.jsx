  ...
  return (
    <Layout>
      <div className="max-w-4xl mx-auto space-y-5">
        ...
        <div className="p-5 rounded-2xl border" style={{ background: "#131315", borderColor: "rgba(255,255,255,0.07)" }}>
          <div className="flex items-start justify-between gap-3 mb-3">
            <div>
              <h1 className="text-xl font-semibold text-white font-display">{gig.title}</h1>
              {gig.description && <p className="text-sm text-zinc-400 mt-1">{gig.description}</p>}
            </div>
          ...
          {gig.data_delivered && (
            <div className="flex items-center gap-2 text-emerald-400 text-xs p-2 rounded-lg mt-2" style={{ background: "rgba(16,185,129,0.1)" }}>
        ...
        <Tabs defaultValue="sessions">
          <TabsList className="bg-zinc-900 border border-white/5">
            <TabsTrigger value="sessions" className="data-[state=active]:bg-amber-500 data-[state=active]:text-black font-display text-xs">Sessions</TabsTrigger>
            <TabsTrigger value="team" className="data-[state=active]:bg-amber-500 data-[state=active]:text-black font-display text-xs">Team ({gig.invites?.length || 0})</TabsTrigger>
            <TabsTrigger value="workspace" className="data-[state=active]:bg-amber-500 data-[state=active]:text-black font-display text-xs">Workspace</TabsTrigger>
          </TabsList>

          {/* Sessions */}
          <TabsContent value="sessions" className="mt-4 space-y-3">
            {gig.sessions?.map((s, i) => (
              <div key={s.id || i} className="p-4 rounded-xl border" style={{ background: "#131315", borderColor: "rgba(255,255,255,0.07)" }}>
                ...
                <div className="flex items-center gap-4 text-sm text-zinc-300 flex-wrap">
                  <span className="flex items-center gap-1.5"><Calendar size={13} className="text-zinc-500" />{s.date}</span>
                  <span className="flex items-center gap-1.5"><Clock size={13} className="text-zinc-500" />{s.start_time} – {s.end_time}</span>
                  <span className="flex items-center gap-1.5"><MapPin size={13} className="text-zinc-500" />{s.location}{s.venue_name && ` · ${s.venue_name}`}</span>
                </div>
              </div>
            ))}
            ...
            {!isLead && myInvite && (
              <div className="p-4 rounded-xl border mt-4" style={{ background: myInvite.status === "pending" ? "rgba(245,158,11,0.05)" : "#131315", borderColor: myInvite.status === "pending" ? "rgba(245,158,11,0.2)" : "rgba(255,255,255,0.07)" }}>
                <p className="text-sm font-semibold text-white font-display mb-1">Your Invite</p>
                <p className="text-xs text-zinc-400 mb-2">{myInvite.role} · Proposed fee: ₹{myInvite.proposed_fee?.toLocaleString("en-IN")}</p>
                {myInvite.status === "pending" && (
                  <div className="flex gap-2">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <Button size="sm" data-testid="counter-offer-btn" onClick={() => handleRespond(myInvite.id, "counter", parseFloat(counterFee[myInvite.id]))} variant="outline" className="h-8 border-purple-500/40 text-purple-400 text-xs gap-1 hover:bg-purple-500/10"><ArrowRightLeft size={12} /> Counter</Button>
                    </div>
                    <Button size="sm" data-testid="reject-invite-btn" onClick={() => handleRespond(myInvite.id, "reject")} variant="outline" className="border-red-500/30 text-red-400 text-xs hover:bg-red-500/10 gap-1"><X size={12} /> Reject</Button>
                  </div>
                )}
          ...
          <TabsContent value="team" className="mt-4">
            {isLead && (
              <div className="flex gap-2 mb-4">
                <Button data-testid="invite-crew-btn" onClick={() => setShowInvite(true)} size="sm" style={{ background: "#F59E0B", color: "#000" }} className="font-display font-semibold gap-1">
                  <UserPlus size={14} /> Invite Crew
                </Button>
                <Button data-testid="ai-suggest-btn" onClick={() => { setShowAI(true); handleAISuggest(); }} size="sm" variant="outline" className="border-purple-500/30 text-purple-400 hover:bg-purple-500/10 gap-1">
            ...
            <div className="space-y-3">
              {gig.invites?.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-zinc-500">No team invites yet</p>
                </div>
              ) : gig.invites?.map(inv => {
                const badge = STATUS_BADGE[inv.status] || STATUS_BADGE.pending;
                return (
                  <div key={inv.id} data-testid={`invite-card-${inv.id}`} className="p-4 rounded-xl border" style={{ background: "#131315", borderColor: "rgba(255,255,255,0.07)" }}>
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <div className="flex items-center gap-2">
                          <p className="text-sm font-semibold text-white font-display">{inv.freelancer?.full_name || "Unknown"}</p>
                          <span className="text-xs px-2 py-0.5 rounded-full font-display" style={badge}>{badge.label}</span>
                        </div>
                        <p className="text-xs text-zinc-400 mt-0.5">{inv.role} · ₹{(inv.agreed_fee || inv.counter_fee || inv.proposed_fee)?.toLocaleString("en-IN")}</p>
                        {inv.status === "counter_offered" && (
                          <p className="text-xs text-purple-400 mt-0.5">Counter: ₹{inv.counter_fee?.toLocaleString("en-IN")}</p>
                        )}
                      </div>
          ...
          <TabsContent value="workspace" className="mt-4">
            <div className="flex justify-end mb-4">
              <Button data-testid="add-workspace-btn" onClick={() => setShowWorkspace(true)} size="sm" style={{ background: "#F59E0B", color: "#000" }} className="font-display gap-1">
                <Upload size={14} /> Add File
              </Button>
            </div>
            {gig.workspace_files?.length === 0 ? (
              <div className="text-center py-12">
                <Upload size={28} className="text-zinc-700 mx-auto mb-2" />
                <p className="text-sm text-zinc-500">No workspace files yet. Add mood boards, call sheets, or location pins.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {gig.workspace_files?.map(f => (
                  <div key={f.id} className="p-4 rounded-xl border" style={{ background: "#131315", borderColor: "rgba(255,255,255,0.07)" }}>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs text-amber-400 font-display uppercase">{f.type}</span>
                    </div>
                    <p className="text-sm text-white font-display font-medium">{f.title}</p>
                    <p className="text-xs text-zinc-400 mt-1 break-all">{f.content}</p>
                  </div>
                ))}
              </div>
            )}
      ...
      <Dialog open={showInvite} onOpenChange={setShowInvite}>
        <DialogContent style={{ background: "#131315", borderColor: "rgba(255,255,255,0.1)" }}>
          <DialogHeader>
            <DialogTitle className="text-white font-display">Invite Crew Member</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 mt-2">
            <div>
              <Label className="text-zinc-300 text-sm font-display">Select Freelancer *</Label>
            ...
            <div>
              <Label className="text-zinc-300 text-sm font-display">Session *</Label>
            ...
            <div>
              <Label className="text-zinc-300 text-sm font-display">Proposed Fee (₹) *</Label>
            ...
            <div className="flex gap-3 pt-1">
              <Button variant="outline" onClick={() => setShowInvite(false)} className="flex-1 border-white/10 text-zinc-400">Cancel</Button>
              <Button data-testid="send-invite-btn" onClick={handleInvite} className="flex-1 font-display" style={{ background: "#F59E0B", color: "#000" }}>Send Invite</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showWorkspace} onOpenChange={setShowWorkspace}>
        <DialogContent style={{ background: "#131315", borderColor: "rgba(255,255,255,0.1)" }}>
          <DialogHeader>
            <DialogTitle className="text-white font-display">Add to Workspace</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 mt-2">
            <div>
              <Label className="text-zinc-300 text-sm font-display">Type</Label>
            ...
            <div>
              <Label className="text-zinc-300 text-sm font-display">Title *</Label>
            ...
            <div>
              <Label className="text-zinc-300 text-sm font-display">Content *</Label>
            ...
            <div className="flex gap-3">
              <Button variant="outline" onClick={() => setShowWorkspace(false)} className="flex-1 border-white/10 text-zinc-400">Cancel</Button>
              <Button data-testid="submit-workspace-btn" onClick={handleAddWorkspace} className="flex-1 font-display" style={{ background: "#F59E0B", color: "#000" }}>Add</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showAI} onOpenChange={setShowAI}>
        <DialogContent className="max-w-lg" style={{ background: "#131315", borderColor: "rgba(255,255,255,0.1)" }}>
          <DialogHeader>
            <DialogTitle className="text-white font-display flex items-center gap-2"><Sparkles size={16} className="text-purple-400" /> AI Crew Suggestions</DialogTitle>
          </DialogHeader>
          {aiLoading ? (
            <div className="flex items-center justify-center py-12">
              <div className="w-6 h-6 border-2 border-purple-400 border-t-transparent rounded-full animate-spin" />
              <span className="ml-3 text-sm text-zinc-400">Generating suggestions...</span>
            </div>
          ) : (
            <div className="mt-2 p-4 rounded-lg text-sm text-zinc-300 leading-relaxed whitespace-pre-wrap" style={{ background: "#1C1C1F" }}>
