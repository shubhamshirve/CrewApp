...
#         app.photoo.in  → 127.0.0.1:3000  (Photoo frontend)
...
header = [
    ...
    "#           for both app.photoo.in and app.mmpf.in.",
...
# ─────────────────────────────────────────────────────────────────────────────
# Auto-pick a free localhost port for the frontend (default 3000; fall back to
# 3100..3199 if something else on the host already owns it). The chosen port
# is also written into the Caddyfile in Step 7 so the host proxy targets it.
FRONTEND_PORT=3000
if ss -tlnH 2>/dev/null | awk '{print $4}' | grep -qE "(:|\.)${FRONTEND_PORT}\$"; then
    warn "Host port ${FRONTEND_PORT} is in use — picking a free alternative..."
    ...
    ok "Selected frontend host port: ${FRONTEND_PORT}"
fi
export FRONTEND_PORT

# Generate a one-off override that injects the chosen port. This sits *next to*
# docker-compose.prod.yml and is regenerated on every run.
cat > docker-compose.runtime.yml <<RUNTIMEEOF
# Auto-generated by deploy.sh — do not edit (regenerated on every run).
services:
  frontend:
    ports: !reset
...
docker compose -f docker-compose.yml -f docker-compose.prod.yml -f docker-compose.runtime.yml down --remove-orphans 2>/dev/null || true
docker compose -f docker-compose.yml -f docker-compose.prod.yml -f docker-compose.runtime.yml up -d --build
ok "Photoo containers up (frontend on 127.0.0.1:${FRONTEND_PORT})"
...
echo ""
info "Local probes:"
curl -fsS -o /dev/null -w "  127.0.0.1:${FRONTEND_PORT}  -> HTTP %{http_code}\n" "http://127.0.0.1:${FRONTEND_PORT}"  || warn "127.0.0.1:${FRONTEND_PORT} down"
