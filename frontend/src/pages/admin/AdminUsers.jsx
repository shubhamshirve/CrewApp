...
export default function AdminUsers() {
  ...
  return (
    <AdminLayout>
            ...
            {users.map(u => (
              ...
              >
                ...
                <div className="flex items-center gap-2 flex-shrink-0 ml-4">
                  ...
                  {u.is_suspended && (
                    <span className="text-[10px] px-2 py-1 rounded-full bg-red-50 text-red-500 border border-red-200">Suspended</span>
                  )}
                  {u.is_featured && (
                    <span className="text-[10px] px-2 py-1 rounded-full bg-amber-50 text-amber-600 border border-amber-200">Featured</span>
                  )}
                  {u.is_high_risk && (
                    <span className="text-[10px] px-2 py-1 rounded-full bg-rose-50 text-rose-500 border border-rose-200">High Risk</span>
                  )}
                  <button
                    data-testid={`penalty-btn-${u.id}`}
                    onClick={() => { setPenaltyData(p => ({ ...p, userId: u.id })); setShowPenalty(true); }}
                    className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs text-orange-500 border border-orange-200 hover:bg-orange-50 transition-colors"
                  >
      ...
      {selected.size > 0 && (
        <div
          className="fixed bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3 px-5 py-3 rounded-2xl border border-slate-200 shadow-2xl z-40 bg-white"
        >
          <span className="text-sm text-slate-700 font-display">{selected.size} selected</span>
          <div className="w-px h-4 bg-slate-200" />
          <button onClick={() => handleBulkAction("suspend")} disabled={bulkLoading} className="text-xs px-3 py-1.5 rounded-lg text-red-500 border border-red-200 hover:bg-red-50 transition-colors disabled:opacity-60">Suspend</button>
          <button onClick={() => handleBulkAction("unsuspend")} disabled={bulkLoading} className="text-xs px-3 py-1.5 rounded-lg text-emerald-600 border border-emerald-200 hover:bg-emerald-50 transition-colors disabled:opacity-60">Unsuspend</button>
          <button onClick={() => handleBulkAction("verify")} disabled={bulkLoading} className="text-xs px-3 py-1.5 rounded-lg text-blue-600 border border-blue-200 hover:bg-blue-50 transition-colors disabled:opacity-60">Verify</button>
          <button onClick={() => setShowNotifyModal(true)} className="flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg text-orange-500 border border-orange-200 hover:bg-orange-50 transition-colors">
            <Send size={11} /> Notify
          </button>
          <button onClick={() => setSelected(new Set())} className="text-xs px-3 py-1.5 rounded-lg text-slate-400 hover:text-slate-600 transition-colors">Clear</button>
        </div>
      )}
        ...
        <DialogContent className="bg-white border-slate-200">
          ...
          <div className="space-y-4 mt-2">
            <div>
              ...
              <select className="w-full rounded-xl px-3 py-2.5 text-sm text-slate-900 border border-slate-200 focus:outline-none bg-white" value={penaltyData.stars} onChange={e => setPenaltyData(p => ({ ...p, stars: parseInt(e.target.value) }))}>
            ...
            <div>
              ...
              <textarea className="w-full rounded-xl px-3 py-2.5 text-sm text-slate-900 border border-slate-200 resize-none h-20 focus:outline-none focus:border-orange-400 bg-white" placeholder="Explain the penalty reason…" value={penaltyData.reason} onChange={e => setPenaltyData(p => ({ ...p, reason: e.target.value }))} />
            </div>
            <div className="flex gap-3">
              <button onClick={() => setShowPenalty(false)} className="flex-1 py-2.5 rounded-xl text-sm text-slate-500 border border-slate-200 hover:bg-slate-50 transition-colors">Cancel</button>
              <button data-testid="apply-penalty-btn" onClick={handlePenalty} className="flex-1 py-2.5 rounded-xl text-sm font-medium text-white transition-opacity hover:opacity-90" style={{ background: "#EA580C" }}>Apply Penalty</button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="bg-white border-slate-200">
          ...
          <div className="space-y-4 mt-2">
            <div>
              ...
              <input className="w-full rounded-xl px-3 py-2.5 text-sm text-slate-900 border border-slate-200 focus:outline-none focus:border-blue-400 bg-white" placeholder="Notification title…" value={notifyForm.title} onChange={e => setNotifyForm(f => ({ ...f, title: e.target.value }))} />
            </div>
            <div>
              <label className="text-xs text-slate-500 mb-1.5 block">Message *</label>
              <textarea className="w-full rounded-xl px-3 py-2.5 text-sm text-slate-900 border border-slate-200 resize-none h-24 focus:outline-none focus:border-blue-400 bg-white" placeholder="Your message…" value={notifyForm.message} onChange={e => setNotifyForm(f => ({ ...f, message: e.target.value }))} />
            </div>
            <div className="flex gap-3">
              <button onClick={() => setShowNotifyModal(false)} className="flex-1 py-2.5 rounded-xl text-sm text-slate-500 border border-slate-200 hover:bg-slate-50 transition-colors">Cancel</button>
